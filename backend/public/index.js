...
document.querySelectorAll('.card,.pack,.usp-item,.step,.test').forEach(el => { el.classList.add('hidden'); observer.observe(el); });
...
  fullscreenButton.addEventListener('click', () => {
    termBox.classList.toggle('is-fullscreen');
    document.body.classList.toggle('terminal-fullscreen-active');
  });
}
...
const heroOrderBtn = document.querySelector(".hero-ctas .btn-primary");
if(heroOrderBtn) {
...
    const service = document.getElementById("contactService").value.trim();
    const message = document.getElementById("contactMessage").value.trim();
    if (!name || !email) { showNotification("⚠️ Name and Email are required!", 6000, true); return; }
    try {
      const res = await fetch("/api/contact", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, email, service, message }) });
      const data = await res.json();
      if (data.status === "success") { contactForm.reset(); showNotification("✅ Your suggestion has been submitted successfully! Thank You."); }
      else { showNotification(data.message || "❌ Failed to submit suggestion", 6000, true); }
    } catch { showNotification("❌ Server error. Try again later.", 6000, true); }
  });
}
...
  const notif = sessionStorage.getItem("notification");
  if (notif) { 
    const isError = notif.startsWith("❌") || notif.startsWith("⚠️");
    showNotification(notif, 6000, isError); 
