...
app.get("/join", (req, res) => {
  res.sendFile(path.join(__dirname, "../frontend/indexJoin.html"));
});
...
app.post("/api/order", upload.single("file"), (req, res) => {
  ...
  transporter.sendMail({
    from: "shauryaprabhakar097@gmail.com",
    to: email,
  ...
  transporter.sendMail({
    from: "shauryaprabhakar097@gmail.com",
    to: "shauryaprabhakar097@gmail.com,itsjatin080@gmail.com",
    subject: "New Order Received - Averian",
    text: `Order ID: ${orderId}\nName: ${name}\nEmail: ${email}\nPhone: ${phone}\nService: ${service}\nDetails: ${details}`,
    attachments: req.file
      ? [{ filename: req.file.originalname, path: path.join(uploadDir, req.file.filename) }]
      : [],
  });
...
app.listen(PORT, () => console.log(`🚀 Averian backend running on http://localhost:${PORT}`));
