...
if (dis) {
  dis.addEventListener("click" , function () {
    alert("Sorry, this function isn't available right now.");
  });
}
...
document.querySelectorAll('.card,.pack,.usp-item,.step,.test,.dashboard .pack').forEach(el=>{
...
/* ---------- REDIRECT BUTTONS ---------- */
document.querySelectorAll("#services, #packages").forEach(btn => {
...
if (contactForm) {
  contactForm.addEventListener("submit", async (e) => {
    ...
    if (!name || !email) {
      showNotification("⚠️ Name and Email are required!", "error");
      return;
    }

    try {
      const res = await fetch("https://nexavian.onrender.com/api/contact", {
        method: "POST",
      ...
      if (data.status === "success") {
        ...
        showNotification(data.message || "❌ Failed to submit suggestion", "error");
      }
    } catch {
